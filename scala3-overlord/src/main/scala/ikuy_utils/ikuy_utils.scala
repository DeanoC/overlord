...
// Using our compatibility layer instead of the original TOML library
...
// In Scala 3, we don't need "extends" for a sealed trait
sealed trait Variant:
  def toTomlString: String
  def toCString: String

// Case classes in Scala 3 style (no parentheses needed for empty parameter lists)
...
case class BooleanV(boolean: Boolean) extends Variant:
  ...
  override def toCString: String = if value then "TRUE" else "FALSE"
...
// Converting the Utils object to Scala 3 syntax
object Utils:
  ...
  def mergeAintoB(a: VariantTable, b: VariantTable): VariantTable =
    a ++ (for s <- b yield if !a.contains(s._1) then Some(s) else None).flatten
  ...
  def readToml(tomlPath: Path): Map[String, Variant] =
    val source = readFile(tomlPath) match
      case Some(value) => value
      case None        =>
        println(s"$tomlPath toml unable to be read")
        return Map[String, Variant]()

    // Using our compatibility layer for TOML parsing
    val tparsed = compat.TomlCompat.Toml.parse(source)
    if tparsed.isLeft then
      println(s"$tomlPath has failed to parse with error ${tparsed.left}")
      Map[String, Variant]()
    else 
      tparsed.toOption.get.values.map(e => e._1 -> toVariant(e._2))

  def lookupString(tbl: VariantTable, key: String, or: String): String =
    if tbl.contains(key) then
  ...
  def lookupStrings(tbl: MutableVariantTable, key: String, or: String): Seq[String] = 
    lookupStrings(tbl.toMap, key, or)
  ...
  def lookupInt(tbl: VariantTable, key: String, or: Int): Int =
    if tbl.contains(key) then Utils.toInt(tbl(key)) else or

  def toInt(t: Variant): Int = t match
    case b: IntV    => b.value
    case b: BigIntV =>
      if b.value < Int.MaxValue && b.value > Int.MinValue then b.value.toInt
      else
  ...
  def lookupDouble(tbl: VariantTable, key: String, or: Double): Double =
    if tbl.contains(key) then Utils.toDouble(tbl(key)) else or
  ...
  def lookupBoolean(tbl: VariantTable, key: String, or: Boolean): Boolean =
    if tbl.contains(key) then Utils.toBoolean(tbl(key)) else or
  ...
  def lookupArray(tbl: VariantTable, key: String): Array[Variant] =
    if tbl.contains(key) then Utils.toArray(tbl(key)) else Array()
  ...
  def lookupBigInt(tbl: VariantTable, key: String, or: BigInt): BigInt =
    if tbl.contains(key) then Utils.toBigInt(tbl(key)) else or
  ...
  private def deleteRecursively(file: File): Unit =
    ...
    if file.exists && !file.delete then
      throw new Exception(s"Unable to delete ${file.getAbsolutePath}")

  def parseBigInt(s: String): Option[BigInt] = Try {
    val ns = s.replace("_", "")

    if ns.startsWith("0x") then Some(BigInt(ns.substring(2), 16))
    else if ns.startsWith("0b") then Some(BigInt(ns.substring(2), 2))
    else Some(BigInt(ns))
  } match
    case Failure(_)     => None
  ...
  def stringToVariant(s: String): Variant =
    ...
    if ns.contains("'") then StringV(s)
    else if ns.contains('"') then StringV(s)
    else if ns == "true" then BooleanV(true)
    else if ns == "false" then BooleanV(false)
