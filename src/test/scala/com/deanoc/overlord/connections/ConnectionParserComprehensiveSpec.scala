...
/**
 * Comprehensive test suite for the ConnectionParser object.
 * This test suite focuses on all aspects of the ConnectionParser's functionality
 * to establish a baseline before refactoring.
 */
class ConnectionParserComprehensiveSpec extends AnyFlatSpec with Matchers with MockitoSugar with SilentLogger {
  ...
  it should "use default values for optional bus parameters" in {
    ...
    val bus = result.get.asInstanceOf[UnconnectedBus]
    bus.busProtocol shouldBe "internal" // default value
    bus.supplierBusName shouldBe "" // default value
    bus.consumerBusName shouldBe "" // should match supplierBusName
  ...
  it should "reject parameters connections without valid first part" in {
    ...
    val invalidParamsList = new java.util.ArrayList[java.util.Map[String, Any]]()
  ...
  it should "handle connection strings with extra whitespace" in {
    ...
    val whitespaceVariant = createConnectionVariant("port", "  device1    ->    device2  ")
  ...
  it should "reject connections with invalid format" in {
    withSilentLogs {
      ...
      val invalidOperatorVariant = createConnectionVariant("port", "device1 => device2")
      ConnectionParser.parseConnection(invalidOperatorVariant) shouldBe None
      
      // Multiple direction operators
      val multipleOperatorsVariant = createConnectionVariant("port", "device1 -> device2 -> device3")
  ...
  "ConnectionParser.parseParametersConnection" should "parse different parameter types" in {
    ...
    // Check parameter names
    result.parameters.map(_.name) should contain allOf("width", "clock_freq", "default_type")
    ...
    val freq = clockParam.parameterType.asInstanceOf[FrequencyParameterType].freq
