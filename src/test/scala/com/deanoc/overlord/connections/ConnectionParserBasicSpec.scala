 ...
 * and demonstrates the use of type-safe configuration classes.
 */
class ConnectionParserBasicSpec extends AnyFlatSpec with Matchers with SilentLogger {
   ...
   * This is kept for backward compatibility testing.
   *
  ...
  /**
   * Helper method to create a test PortConnectionConfig for connection testing.
   *
   * @param connection The connection string (e.g., "device1 -> device2")
   * @return A PortConnectionConfig containing the connection information
   */
   ...
   *
   * @param connection The connection string (e.g., "device1 -> device2")
   * @param busProtocol The bus protocol (e.g., "axi")
   * @param busName The bus name (optional)
   * @return A BusConnectionConfig containing the connection information
   */
