...
import com.deanoc.overlord.utils.{SilentLogger, Logging}
 ...
 */
class VerilogGitHubIntegrationTest extends AnyFlatSpec with Matchers with SilentLogger with Logging {
  ...
  private def downloadFile(url: String, targetPath: String): Boolean = {
    try {
      ...
      case e: Exception => 
        error(s"Error downloading $url: ${e.getMessage}")
        false
    }
  }
    ...
    withSilentLogs {
      ...
      for ((url, i) <- githubFiles.zipWithIndex) {
        ...
        if (downloadFile(url, filePath)) {
          ...
          if (result.isRight) {
            val modules = result.getOrElse(Seq.empty)
            info(s"Successfully parsed ${modules.length} modules from $url")
            modules.foreach { m =>
              info(s"  Module: ${m.name}")
              info(s"    Ports: ${m.module_boundary.collect { case p: VerilogPort => p }.length}")
              info(s"    Parameters: ${m.module_boundary.collect { case p: VerilogParameterKey => p }.length}")
            }
            successCount += 1
          } else {
            error(s"Failed to parse $url: ${result.left.getOrElse("Unknown error")}")
          }
        }
      }
