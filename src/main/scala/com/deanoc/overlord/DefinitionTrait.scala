...
import io.circe.{Json, JsonObject, Encoder} // Import Json, JsonObject, and Encoder
...
object Definition {
  def apply(
    ...
    defType match {
      case dt: ChipDefinitionType     => HardwareDefinition(defType, mergedConfig, path)
      case dt: SoftwareDefinitionType => SoftwareDefinition(defType, mergedConfig, path)
      case dt: PinGroupDefinitionType => HardwareDefinition(defType, mergedConfig, path)
      case dt: ClockDefinitionType    => HardwareDefinition(defType, mergedConfig, path)
      case dt: BoardDefinitionType    => HardwareDefinition(defType, mergedConfig, path)
      case _                          => Left(s"Unknown definition type: ${config.`type`}")
    }
  }
  ...
  def anyToJson(value: Any): Json = value match {
    ...
    case map: Map[_, _] =>
      Json.fromJsonObject(JsonObject.fromIterable(map.map {
        ...
        case (k, v) => k.toString -> anyToJson(v) // Convert non-String keys to String
      }))
    case list: List[_] => Json.fromValues(list.map(anyToJson))
    case seq: Seq[_] => Json.fromValues(seq.map(anyToJson))
    case other => Json.fromString(other.toString) // Convert other types to String
  }
}
  ...
  def createInstance(
    ...
    defType match {
      case _: RamDefinitionType =>
        instanceConfig match {
          case Some(configMap) =>
            ...
            io.circe.parser.decode[RamConfig](Definition.anyToJson(configMap).noSpaces) match { // Corrected decoding using helper
              case Right(ramConfig) =>
                RamInstance(name, this, ramConfig).asInstanceOf[Either[String, InstanceTrait]] // Cast to expected return type
              case Left(error) =>
                Left(s"Failed to decode RamConfig for instance $name: ${error.getMessage}")
            }
          case None => Left(s"RamDefinitionType instance $name requires configuration")
        }
      case _: CpuDefinitionType =>
        instanceConfig match {
          case Some(configMap) =>
            // Attempt to decode the config map into CpuConfig
            io.circe.parser.decode[CpuConfig](Definition.anyToJson(configMap).noSpaces) match { // Corrected decoding using helper
              case Right(cpuConfig) =>
                CpuInstance(name, this, cpuConfig).asInstanceOf[Either[String, InstanceTrait]] // Cast to expected return type
              case Left(error) =>
                Left(s"Failed to decode CpuConfig for instance $name: ${error.getMessage}")
            }
          case None => Left(s"CpuDefinitionType instance $name requires configuration")
        }
      case _: IoDefinitionType =>
        instanceConfig match {
          case Some(configMap) =>
            // Attempt to decode the config map into IoConfig
            io.circe.parser.decode[IoConfig](Definition.anyToJson(configMap).noSpaces) match { // Corrected decoding using helper
              case Right(ioConfig) =>
                IoInstance(name, this, ioConfig).asInstanceOf[Either[String, InstanceTrait]] // Cast to expected return type
              case Left(error) =>
                Left(s"Failed to decode IoConfig for instance $name: ${error.getMessage}")
            }
          case None => Left(s"IoDefinitionType instance $name requires configuration")
        }
      case _: PinGroupDefinitionType =>
        instanceConfig match {
          case Some(configMap) =>
            // Attempt to decode the config map into PinGroupConfig
            io.circe.parser.decode[PinGroupConfig](Definition.anyToJson(configMap).noSpaces) match { // Corrected decoding using helper
              case Right(pinGroupConfig) =>
                PinGroupInstance(name = name, definition = this, config = pinGroupConfig).asInstanceOf[Either[String, InstanceTrait]] // Cast to expected return type
              case Left(error) =>
                Left(s"Failed to decode PinGroupConfig for instance $name: ${error.getMessage}")
            }
          case None => Left(s"PinGroupDefinitionType instance $name requires configuration")
        }
      case _: ClockDefinitionType =>
        instanceConfig match {
          case Some(configMap) =>
            // Attempt to decode the config map into ClockConfig
            io.circe.parser.decode[ClockConfig](Definition.anyToJson(configMap).noSpaces) match { // Corrected decoding using helper
              case Right(clockConfig) =>
                ClockInstance(name, this, clockConfig).asInstanceOf[Either[String, InstanceTrait]] // Cast to expected return type
              case Left(error) =>
                Left(s"Failed to decode ClockConfig for instance $name: ${error.getMessage}")
            }
          case None => Left(s"ClockDefinitionType instance $name requires configuration")
        }
      case _: BoardDefinitionType =>
        instanceConfig match {
          case Some(configMap) =>
            // Attempt to decode the config map into BoardConfig
            io.circe.parser.decode[BoardConfig](Definition.anyToJson(configMap).noSpaces) match { // Corrected decoding using helper
              case Right(boardConfig) => BoardInstance(name = name, definition = this, config = boardConfig) // Call apply method with named parameters
              case Left(error)        => Left(s"Failed to decode BoardConfig for instance $name: ${error.getMessage}")
            }
          case None => Left(s"BoardDefinitionType instance $name requires configuration")
        }
      ...
      case _: GraphicDefinitionType  => { // Manually create instance and merge attributes
        ...
        val attribs = instanceConfig.getOrElse(Map[String, Any]()).map { case (k, v) => k -> com.deanoc.overlord.utils.Utils.toVariant(v) }
      ...
      case _: LibraryDefinitionType =>
        instanceConfig match {
          case Some(configMap) =>
            ...
            io.circe.parser.decode[LibraryConfig](Definition.anyToJson(configMap).noSpaces) match { // Corrected decoding using helper
              case Right(libraryConfig) =>
                LibraryInstance(name, this, libraryConfig).asInstanceOf[Either[String, InstanceTrait]] // Cast to expected return type
              case Left(error) =>
                Left(s"Failed to decode LibraryConfig for instance $name: ${error.getMessage}")
            }
          case None => Left(s"LibraryDefinitionType instance $name requires configuration")
        }
      case _: ProgramDefinitionType =>
        instanceConfig match {
          case Some(configMap) =>
            // Attempt to decode the config map into ProgramConfig
            io.circe.parser.decode[ProgramConfig](Definition.anyToJson(configMap).noSpaces) match { // Corrected decoding using helper
              case Right(programConfig) =>
                ProgramInstance(name, this, programConfig).asInstanceOf[Either[String, InstanceTrait]] // Cast to expected return type
              case Left(error) =>
                Left(s"Failed to decode ProgramConfig for instance $name: ${error.getMessage}")
            }
          case None => Left(s"ProgramDefinitionType instance $name requires configuration")
        }
