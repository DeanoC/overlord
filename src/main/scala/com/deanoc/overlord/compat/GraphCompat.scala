package com.deanoc.overlord.compat
...
/** Compatibility layer for scala-graph using JGraphT. This provides a bridge
  * between the original scala-graph API used in Scala 2 and the JGraphT library
  * used in Scala 3.
  */
object GraphCompat:
  ...
  class Graph[N, E]:
    private val graph =
      new DirectedMultigraph[N, LabeledEdge[N, E]](classOf[LabeledEdge[N, E]])
    ...
    def outEdges(node: N): Set[(N, N, E)] =
      graph
        ...
        .asScala
        .map(e => (e.getSource, e.getTarget, e.getLabel))
        .toSet
