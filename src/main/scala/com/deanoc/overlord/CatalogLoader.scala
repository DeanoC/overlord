...
import com.deanoc.overlord.utils.{Logging, Utils, Variant} // Import Utils and Variant
import com.deanoc.overlord.config.{CatalogFileConfig, DefinitionConfig, CatalogSourceConfig, ProjectFileConfig} // Import catalog config case classes
...
object CatalogLoader extends Logging {
   ...
   def parseDefinitionCatalog(
       ...
       defaultMap: Map[String, Any] // Changed to Any as defaults can be various types
       ...
       for (definitionConfig <- definitions) {
         ...
         Definition(definitionConfig, defaultsAsVariant) match {
           ...
           case Left(errorMsg) => 
             ...
             this.error(s"Error creating definition ${definitionConfig.name}: $errorMsg", null)
         }
       }
     }
   ...
   private def processCatalogSource(
    ...
    val contentPath = sourceType match {
      case "git" | "fetch" => sourceConfig.url.getOrElse {
        error(s"Catalog source of type '$sourceType' missing 'url'")
        boundary.break(Seq.empty)
      }
      case "local"         => sourceConfig.path.getOrElse {
  ...
  private def loadFromFile(
    ...
    val filePath = Overlord.projectPath.resolve(fileName) // Assuming local paths are relative to project root
    ...
    parsedConfig match {
      ...
      case Right(parsed) =>
        ...
        val result = parseDefinitionCatalog(filePath.toString, parsed, defaultMap) // parsed is already CatalogFileConfig
                ...
                for (include <- includes) {
                  ...
                  if (includedPath.isEmpty) {
                    ...
                    prefabCatalog.loadPrefabFromFile(includedPath.get) match {
                      case Left(err) =>
                        warn(s"Failed to load included prefab $includedName: $err")
