...
object BoardInstance {
  ...
  def apply(
    ...
    val clocksResult = Utils
      .toArray(attribs("clocks"))
      .foldLeft[Either[String, Seq[InstanceTrait]]](Right(Seq.empty)) {
        ...
        case (Right(clocksAcc), pinv) =>
          ...
          if (table.contains("name")) {
            ...
            Definition(TableV(clock), defaults)
              .createInstance(s"$name", clock) match {
    ...
    val clocks =
      clocksResult.toOption.get // Instantiate all pingroups defined in the attributes.
    val pingroupsResult = Utils
      .toArray(attribs("pingroups"))
      .foldLeft[Either[String, Seq[InstanceTrait]]](Right(Seq.empty)) {
        case (Left(error), _) => Left(error)
