...
object DistanceMatrix {
	...
	def apply(instances: Seq[ChipInstance], connected: Seq[Connected]): DistanceMatrix = {
	...
	private def flattenInstances(instances: Seq[ChipInstance]): Array[ChipInstance] = {
	...
	case class Dijkstra() {
		...
		def solve(nodes: Seq[Node],
		          sourceIndex: Int,
		          destIndex: Int): Option[(Cost, IndexedSeq[Node])] = {
				...
				if (!visited.contains(currentNode)) {
					...
					if (currentNode == dest) {
						...
						neighboursWithCost.foreach { case (neighbourIndex, neighbourCost) =>
							...
							heap +=
							((currentCost + neighbourCost, neighbourNode, currentPath :+
							                                              neighbourNode))
						}
					}
				}
