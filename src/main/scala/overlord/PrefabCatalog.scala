...
object PrefabCatalog {
	...
	def fromFile(fileName: String): Seq[Prefab] = {
		...
		if (Files.size(filePath.toAbsolutePath) == 0) {
			...
			return Seq(Prefab(name, Project.instancePath.toString, Map.empty, Map.empty))
		}
		...
		if (source == null) {
			println(s"$fileName contains only comments or is invalid, creating an empty prefab")
			val name = fileName.replace("/", ".")
		...
		if (prefabs.isEmpty) {
			...
			val name = fileName.replace("/", ".")
			prefabs ++= Seq(Prefab(name, Project.instancePath.toString, Map.empty, Map.empty))
		}
