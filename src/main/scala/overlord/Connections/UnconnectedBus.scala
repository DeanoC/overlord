...
import overlord.Interfaces.{PortsLike, RamLike, SupplierBusLike, MultiBusLike}
...
case class UnconnectedBus(firstFullName: String,
	...
	override def preConnect(unexpanded: Seq[ChipInstance]): Unit = {
		...
		val (bus: SupplierBusLike, other: ChipInstance) = getBus(mainIL, otherIL, direction) match {
	...
	private def supplierToConsumerHookup(bus: SupplierBusLike, other: ChipInstance): Unit = {
		if (other.hasInterface[RamLike]) {
			...
			ram.getRanges.foreach { case (address, size, _, _) => {
	...
	private def getBus(mainIL: InstanceLoc,
	                   otherIL: InstanceLoc,
	                   direction: ConnectionDirection): Option[(SupplierBusLike, ChipInstance)] = {
		/*
		...
		val supplierBus: SupplierBusLike = {
			...
			if (byName.nonEmpty) byName.get
				...
				println(s"ERROR: $firstFullName and $secondFullName supplioer doesn't have a $supplierBusName named bus")
				return None
			}
		}
		...
		if (consumerMultiBusesO.nonEmpty) {
			...
			if (byName.isEmpty && busProtocol.nonEmpty) {
				...
				println(s"ERROR: $firstFullName and $secondFullName consumer doesn't have a $consumerBusName named bus")
				return None
			}
		}
	...
	override def connect(unexpanded: Seq[ChipInstance]): Seq[Connected] = {
		...
		val busConnection = Seq[Connected](ConnectedBus(GroupConnectionPriority(), mainIL, direction, otherIL, bus, other))
