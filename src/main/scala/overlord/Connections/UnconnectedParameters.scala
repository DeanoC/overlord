...
case class UnconnectedParameters(direction: ConnectionDirection,
                                 instanceName: String,
                                 parameters: Seq[Parameter]) extends Unconnected {
	...
	override def collectConstants(unexpanded: Seq[InstanceTrait]): Seq[Constant] = {
		...
		for {instance <- instances
		     param <- parameters} yield Constant(instance.instance, param)
	}
	...
	def apply(direction: ConnectionDirection, secondFullName: String, parametersV: Array[Variant]): Unconnected = {
		val parameters = parametersV.flatMap { v =>
			...
			if (!table.contains("name")) {
				println(s"ERROR: parameter table entry does have a name _ -> $secondFullName")
				None
			} else {
				val name = Utils.lookupString(table, "name", "NO_NAME")
				Utils.lookupString(table, "type", "_") match {
					case "constant"  => Some(Parameter(name, ConstantParameterType(table("value"))))
					case "frequency" => Some(Parameter(name, FrequencyParameterType(Utils.toFrequency(table("value")))))
					case _           => None
				}
			}
		}
