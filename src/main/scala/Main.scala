...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (
      !options.contains(Symbol("nostdresources")) &&
      (!Files.exists(Resources.stdResourcePath()) || !isValidGitRepo(
        Resources.stdResourcePath()
      ))
    ) {
      ...
      if (shouldDownload) {
        ...
        val cloneCommand =
          s"git clone https://github.com/DeanoC/gagameos_stdcatalog.git ${Resources.stdResourcePath()}"
    ...
    val game = Project(
      gameName,
      options(Symbol("board")).asInstanceOf[String],
      filePath,
      chipCatalog,
      prefabCatalog
    ) match {
