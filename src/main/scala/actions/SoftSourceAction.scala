      ...
      try {
        ...
        val missingFields = srcs.zipWithIndex.collectFirst {
          case (entry, idx) if !entry.contains("in") => 
            Left(s"SoftSourceAction entry #${idx+1} in $name is missing the 'in' field")
          case (entry, idx) if !entry.contains("out") => 
            Left(s"SoftSourceAction entry for ${idx+1} in $name is missing the 'out' field")
        }
              ...
              val phaseAndCpus: (Int, Seq[String]) = {
                if (entry.contains("cpus")) {
                  ...
                  if (cpusString == "_") (2, Seq()) // Phase 2 with no CPU targets
                    ...
                    val cpusList = cpuRegEx.split(cpusString).map(_.toLowerCase())
                    if (allCpus.isDefined) (2, cpusList.intersect(allCpus.get).toSeq)
        ...
        case e: Exception => Left(s"Error processing software sources in $name: ${e.getMessage}")
      }
    }
  }
