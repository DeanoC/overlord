...
LONG XEmacPs_SetHandler(XEmacPs *InstancePtr, u32 HandlerType,
			void (*FuncPointer)(void*), void *CallBackRef)
{
	LONG Status;
	...
	case XEMACPS_HANDLER_DMASEND:
		...
		InstancePtr->SendHandler = FuncPointer;
		InstancePtr->SendRef = CallBackRef;
		break;
	case XEMACPS_HANDLER_DMARECV:
		Status = (LONG)(XST_SUCCESS);
		InstancePtr->RecvHandler = FuncPointer;
		InstancePtr->RecvRef = CallBackRef;
		break;
	case XEMACPS_HANDLER_ERROR:
		Status = (LONG)(XST_SUCCESS);
		InstancePtr->ErrorHandler = (XEmacPs_ErrHandler)FuncPointer;
