		...
		for (Int_Id = 0U; Int_Id < XSCUGIC_MAX_NUM_INTR_INPUTS;
			...
			*/
			if ((InstancePtr->Config->HandlerTable[Int_Id].Handler
					== (XInterruptHandler)NULL)) {
				InstancePtr->Config->HandlerTable[Int_Id].Handler
						= (XInterruptHandler)StubHandler;
			}
				...
				XInterruptHandler Handler, void *CallBackRef)
{
	 ...
	 * handler
	 */
	InstancePtr->Config->HandlerTable[Int_Id].Handler = (XInterruptHandler)Handler;
...
void XScuGic_Disconnect(XScuGic *InstancePtr, u32 Int_Id)
{
	 ...
	 * tracked
	 */
	InstancePtr->Config->HandlerTable[Int_Id].Handler = (XInterruptHandler)StubHandler;
