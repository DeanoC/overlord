...
#define XAVBUF_PLL_OUT_FREQ		1500000000
...
void Init(Connection *link) {
	...
	HW_REG_SET_BIT(DP, SOFTWARE_RESET, SOFT_RST);
	...
	HW_REG_SET_BIT(DP, MAIN_STREAM_ENABLE, MS_ENABLE);
	...
	HW_REG_SET_BIT(DP, INT_EN, HPD_PULSE_DET);
	HW_REG_SET_BIT(DP, INT_EN, HPD_EVENT);
	HW_REG_SET_BIT(DP, INT_EN, HPD_IRQ);
...
bool IsDisplayConnected(Connection *link) {
	...
	do {
		bool const hpdState = HW_REG_GET_BIT(DP, INTERRUPT_SIGNAL_STATE, HPD_STATE);
		bool const hpdEvent = HW_REG_GET_BIT(DP, INT_STATUS, HPD_EVENT);
		bool const hpdPulseDetected = HW_REG_GET_BIT(DP, INT_STATUS, HPD_PULSE_DET);
		uint32_t hpdPulseDuration = 0;
		if (hpdPulseDetected) {
			hpdPulseDuration = HW_REG_GET(DP, HPD_DURATION);
		}
...
void EnableMainLink(bool enable) {
	HW_REG_SET(DP, FORCE_SCRAMBLER_RESET, 1);
	HW_REG_SET(DP, MAIN_STREAM_ENABLE, enable);
}
...
void SetDisplay(Connection *link, Display *display, Mixer *mixer) {
	...
	uint32_t userDataCountPerLane = wordsPerLine - link->numLanes;
	if ((wordsPerLine % link->numLanes) != 0) {
		userDataCountPerLane += wordsPerLine % link->numLanes;
	}
	...
	uint32_t const linkBandwidth = link->numLanes * (int) link->linkRate * 27;
	uint32_t const vl = ((10 * (videoBandwidth * transferUnitSize) / linkBandwidth) + 5) / 10;
	uint32_t const minBytesPerTransferUnit = vl / 1000;
	uint32_t const fractionBytesPerTransferUnit = (vl % 1000) * 1000;

	uint32_t const mClockFactor = pixelClockKHz;
	uint32_t const nClockFactor = 27000 * (int) link->linkRate;
...
*/
	...
	PSI_IWord const setMainStreamOutputProgram[] = {
			...
			PSI_WRITE_32(DP, USER_PIX_WIDTH, 1),
	...
	HW_REG_SET(DP, SOFTWARE_RESET, HW_REG_ENCODE_FIELD(DP, SOFTWARE_RESET, SOFT_RST, 1));
	Utils_BusyMicroSleep(10);
	HW_REG_SET(DP, SOFTWARE_RESET, HW_REG_ENCODE_FIELD(DP, SOFTWARE_RESET, SOFT_RST, 0));
...
void SetMixerDMA(Mixer* mixer) {
	...
	HW_REG_SET(DP, INT_MASK, 0xFFFFFFFF);
	HW_REG_SET(DP, INT_STATUS, 0xFFFFFFFF);
	HW_REG_SET(DPDMA, IMR, 0xFFFFFFFF);
	HW_REG_SET(DPDMA, EIMR, 0xFFFFFFFF);
	HW_REG_SET(DPDMA, ISR, 0xFFFFFFFF);
	HW_REG_SET(DPDMA, EISR, 0xFFFFFFFF);

	HW_REG_SET(DP, AV_BUF_FORMAT,
	...
	HW_REG_SET(DP, AV_BUF_AUD_VID_CLK_SOURCE,
	...
	HW_REG_SET(DP, AV_CHBUF0, HW_REG_ENCODE_FIELD(DP, AV_CHBUF0, FLUSH, 1));
	HW_REG_SET(DP, AV_CHBUF1, HW_REG_ENCODE_FIELD(DP, AV_CHBUF1, FLUSH, 1));
	HW_REG_SET(DP, AV_CHBUF2, HW_REG_ENCODE_FIELD(DP, AV_CHBUF2, FLUSH, 1));
	HW_REG_SET(DP, AV_CHBUF3, HW_REG_ENCODE_FIELD(DP, AV_CHBUF3, FLUSH, 1));

	if(planeEnable[0]) {
		HW_REG_SET(DP, AV_CHBUF0,
							 HW_REG_ENCODE_FIELD(DP, AV_CHBUF0, BURST_LEN, 15) |
							 HW_REG_ENCODE_FIELD(DP, AV_CHBUF0, EN, 1));
	}
	if(planeEnable[1]) {
		HW_REG_SET(DP, AV_CHBUF1,
							 HW_REG_ENCODE_FIELD(DP, AV_CHBUF1, BURST_LEN, 15) |
							 HW_REG_ENCODE_FIELD(DP, AV_CHBUF1, EN, 1));
	}
	if(planeEnable[2]) {
		HW_REG_SET(DP, AV_CHBUF2,
							 HW_REG_ENCODE_FIELD(DP, AV_CHBUF2, BURST_LEN, 15) |
							 HW_REG_ENCODE_FIELD(DP, AV_CHBUF2, EN, 1));
	}
	if(planeEnable[3]) {
		HW_REG_SET(DP, AV_CHBUF3,
							 HW_REG_ENCODE_FIELD(DP, AV_CHBUF3, BURST_LEN, 15) |
							 HW_REG_ENCODE_FIELD(DP, AV_CHBUF3, EN, 1));
	}

	HW_REG_SET(DP, AV_BUF_VIDEO_COMP0_SCALE_FACTOR,
						 HW_REG_ENCODE_FIELD(DP, AV_BUF_VIDEO_COMP0_SCALE_FACTOR, VID_SCA_FACT0, 0x10101) );
	HW_REG_SET(DP, AV_BUF_VIDEO_COMP1_SCALE_FACTOR,
	...
	HW_REG_SET(DP, AV_BUF_GRAPHICS_COMP0_SCALE_FACTOR,
						 HW_REG_ENCODE_FIELD(DP, AV_BUF_GRAPHICS_COMP0_SCALE_FACTOR, GRAPHICS_SCALE_FACTOR0, 0x10101) );
	HW_REG_SET(DP, AV_BUF_GRAPHICS_COMP1_SCALE_FACTOR,
	...
	HW_REG_SET(DP, AV_BUF_SRST_REG, HW_REG_ENCODE_FIELD(DP, AV_BUF_SRST_REG, VID_RST, 1));
	Utils_BusyMicroSleep(10);
	HW_REG_SET(DP, AV_BUF_SRST_REG, HW_REG_ENCODE_FIELD(DP, AV_BUF_SRST_REG, VID_RST, 0));

	HW_REG_SET(DPDMA, CH0_CNTL, HW_REG_ENCODE_FIELD(DPDMA, CH0_CNTL, EN, 0));
	HW_REG_SET(DPDMA, CH1_CNTL, HW_REG_ENCODE_FIELD(DPDMA, CH1_CNTL, EN, 0));
	HW_REG_SET(DPDMA, CH2_CNTL, HW_REG_ENCODE_FIELD(DPDMA, CH2_CNTL, EN, 0));
	HW_REG_SET(DPDMA, CH3_CNTL, HW_REG_ENCODE_FIELD(DPDMA, CH3_CNTL, EN, 0));

	if(planeEnable[0]) {
		assert(mixer->videoPlane.simpleDescPlane0Address);

		HW_REG_SET(DPDMA, CH0_DSCR_STRT_ADDRE,
	...
	if(planeEnable[1]) {
		...
		HW_REG_SET(DPDMA, CH1_DSCR_STRT_ADDRE,
	...
	if(planeEnable[2]) {
		...
		HW_REG_SET(DPDMA, CH2_DSCR_STRT_ADDRE,
	...
	if(planeEnable[3]) {
		...
		HW_REG_SET(DPDMA, CH3_DSCR_STRT_ADDRE,
	...
	HW_REG_SET(DP, AV_BUF_OUTPUT_AUDIO_VIDEO_SELECT,
...
/*
	for(int i = 0; i < 10;i++) {
		debug_printf("DP_INT_STATUS 0x%x\n", HW_REG_GET(DP, INT_STATUS));
		debug_printf("DPDMA_ISR 0x%x\n", HW_REG_GET(DPDMA, ISR));
		debug_printf("DPDMA_EISR 0x%x\n", HW_REG_GET(DPDMA, EISR));
		HW_REG_SET(DP, INT_STATUS, HW_REG_GET(DP, INT_STATUS));
		HW_REG_SET(DPDMA, ISR, HW_REG_GET(DPDMA, ISR));
		HW_REG_SET(DPDMA, EISR, HW_REG_GET(DPDMA, EISR));
