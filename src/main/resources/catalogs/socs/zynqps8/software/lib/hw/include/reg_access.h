...
ALWAYS_INLINE volatile uint32_t hw_reg_read(const uintptr_t addr, const uint32_t offset_in_bytes) { return *(((const volatile uint32_t *)addr) + offset_in_bytes/sizeof(uint32_t)); }
ALWAYS_INLINE void hw_reg_write(const uintptr_t addr, const uint32_t offset_in_bytes, const uint32_t value) { *(((volatile uint32_t *)addr) + offset_in_bytes/sizeof(uint32_t)) = value; }

#define HW_REG_GET(type, reg) hw_reg_read(type##_BASE_ADDR, type##_REGISTER(reg))
#define HW_REG_SET(type, reg, value) hw_reg_write(type##_BASE_ADDR, type##_REGISTER(reg), (value))

#define HW_REG_SET_BIT(type, reg, field) HW_REG_SET( type, reg, ((HW_REG_GET(type, reg) & ~HW_REG_FIELD_MASK(type, reg, field)) | HW_REG_FIELD_MASK(type, reg, field)) )
