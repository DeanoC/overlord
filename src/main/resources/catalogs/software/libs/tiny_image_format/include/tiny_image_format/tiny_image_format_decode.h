...
ALWAYS_INLINE void TinyImageFormat_SharedE5B9G9R9UFloatToFloats(uint32_t v, float out[4]) {
...
ALWAYS_INLINE float TinyImageFormat_HalfAsUintToFloat(uint16_t h_) {
...
ALWAYS_INLINE float TinyImageFormat_BFloatAsUintToFloat(uint16_t h_) {
	union {
			struct {
					uint16_t x;
					uint16_t u;
...
ALWAYS_INLINE float TinyImageFormat_UFloat6AsUintToFloat(uint16_t Value) {
	// https://github.com/microsoft/DirectXMath/blob/ecfb4754400dac581c2eeb6e849617cf5d210426/Inc/DirectXPackedVector.h
...
ALWAYS_INLINE float TinyImageFormat_UFloat7AsUintToFloat(uint16_t Value) {
	// https://github.com/microsoft/DirectXMath/blob/ecfb4754400dac581c2eeb6e849617cf5d210426/Inc/DirectXPackedVector.h
...
ALWAYS_INLINE float TinyImageFormat_UFloat10AsUintToFloat(uint16_t v) {
	// https://github.com/microsoft/DirectXMath/blob/ecfb4754400dac581c2eeb6e849617cf5d210426/Inc/DirectXPackedVector.h
	union {
			struct {
					uint16_t e: 5;
...
ALWAYS_INLINE float TinyImageFormat_UFloat11AsUintToFloat(uint16_t v) {
	// https://github.com/microsoft/DirectXMath/blob/ecfb4754400dac581c2eeb6e849617cf5d210426/Inc/DirectXPackedVector.h
	union {
			struct {
					uint16_t e: 5;
...
TIF_CONSTEXPR ALWAYS_INLINE float TinyImageFormat_LookupSRGB(uint8_t lookup) {
	switch (lookup) {
		case 0:
			return 0.00000000f;
		case 1:
...
TIF_CONSTEXPR ALWAYS_INLINE bool TinyImageFormat_CanDecodeLogicalPixelsF(TinyImageFormat const fmt) {
	switch (fmt) {
...
TIF_CONSTEXPR ALWAYS_INLINE bool
TinyImageFormat_DecodeLogicalPixelsF(TinyImageFormat const fmt, TinyImageFormat_DecodeInput *in, uint32_t const width,
                                     float *out) {
	switch (fmt) {
		case TinyImageFormat_R1_UNORM:
...
TIF_CONSTEXPR ALWAYS_INLINE bool TinyImageFormat_CanDecodeLogicalPixelsD(TinyImageFormat const fmt) {
	switch (fmt) {
		...
		case TinyImageFormat_R64G64B64A64_SFLOAT:
			return true;
		default:
			return TinyImageFormat_CanDecodeLogicalPixelsF(fmt);
	}
}

TIF_CONSTEXPR ALWAYS_INLINE bool
TinyImageFormat_DecodeLogicalPixelsD(TinyImageFormat const fmt, TinyImageFormat_DecodeInput *in, uint32_t const width,
                                     double *out) {
	switch (fmt) {
		case TinyImageFormat_R64_UINT:
			for (uint32_t w = 0; w < width; ++w) {
