...
ALWAYS_INLINE void RequestPrefetch(const uint8_t* address) {
  __builtin_prefetch((const char*)(address), 0 /* Read only. */, 0 /* No temporal locality. */);
}
// Returns the smallest number >= the given number that is evenly divided by N.
//
// N must be a power of two.
ALWAYS_INLINE uintptr_t RoundUp8(uintptr_t pointer) {
	static_assert((8 & (8 - 1)) == 0, "N must be a power of two");
	return (pointer + (8 - 1)) & ~(8 - 1);
}
...
const ptrdiff_t kGroups = 3;
const ptrdiff_t kBlock0Size = 16 * 1024 / kGroups / 64 * 64;
const ptrdiff_t kBlock1Size = 4 * 1024 / kGroups / 8 * 8;
const ptrdiff_t kBlock2Size = 1024 / kGroups / 8 * 8;
...
const ptrdiff_t kPrefetchHorizon = 256;
...
#define SKIP_BLOCK(crc, tab)                                      \
  ...
  // end of the buffer.
  const uint8_t* x = (const uint8_t*) RoundUp8((uintptr_t)p);
  if (x <= e) {
