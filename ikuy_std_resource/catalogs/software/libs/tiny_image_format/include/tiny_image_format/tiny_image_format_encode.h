...
ALWAYS_INLINE uint8_t TinyImageFormat_FloatToSRGB(float v) {
	if (v < 0.0031308f) {
		v *= 12.92f;
	} else {
...
ALWAYS_INLINE uint16_t TinyImageFormat_FloatToHalfAsUint(float f_) {
	union {
			uint16_t u;
			struct {
...
ALWAYS_INLINE uint16_t TinyImageFormat_FloatToBFloatAsUint(float v) {
	union {
			struct {
					uint16_t x;
					uint16_t u;
...
ALWAYS_INLINE void TinyImageFormat_FloatRGBToRGB9E5AsUint32(float const *in, uint32_t *out) {
	float const r = in[0];
	float const g = in[1];
	float const b = in[2];
...
ALWAYS_INLINE uint16_t TinyImageFormat_FloatToUFloat6AsUint(float Value) {
...
ALWAYS_INLINE uint16_t TinyImageFormat_FloatToUFloat7AsUint(float Value) {
	uint32_t IValue = *((uint32_t *) &Value);
...
ALWAYS_INLINE uint16_t TinyImageFormat_FloatToUFloat10AsUint(float v) {
	union {
			uint32_t u;
			float f;
			struct {
...
ALWAYS_INLINE uint16_t TinyImageFormat_FloatToUFloat11AsUint(float v) {
	union {
			uint32_t u;
			float f;
			struct {
...
TIF_CONSTEXPR ALWAYS_INLINE bool TinyImageFormat_CanEncodeLogicalPixelsF(TinyImageFormat const fmt) {
	switch (fmt) {
...
TinyImageFormat_EncodeLogicalPixelsF(TinyImageFormat const fmt, float const *in, uint32_t const width,
...
TIF_CONSTEXPR ALWAYS_INLINE bool TinyImageFormat_CanEncodeLogicalPixelsD(TinyImageFormat const fmt) {
	switch (fmt) {
		...
		case TinyImageFormat_R64G64B64A64_SFLOAT:
			return true;
		default:
			return TinyImageFormat_CanEncodeLogicalPixelsF(fmt);
	}
}

TIF_CONSTEXPR ALWAYS_INLINE bool
TinyImageFormat_EncodeLogicalPixelsD(TinyImageFormat const fmt, double const *in, uint32_t const width,
                                     TinyImageFormat_EncodeOutput *out) {
	switch (fmt) {
		case TinyImageFormat_R64_UINT:
			for (uint32_t w = 0; w < width; ++w) {
